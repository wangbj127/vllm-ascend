...
std::tuple<at::Tensor, at::Tensor> npu_sparse_flash_mla_npu(
    ...
    const c10::string_view device)
{
    ...
    for (const auto *tensor_opt : device_inputs) {
        if (tensor_opt->has_value()) {
            output_device = tensor_opt->value().device();
            break;
        }
    }
    at::Tensor output = torch::empty({OUTPUT_SIZE}, torch::dtype(torch::kInt32).device(output_device));
    ...
    EXEC_NPU_CMD(aclnnSparseFlashMlaMetadata, cu_seqlens_q_val, cu_seqlens_ori_kv_val,
        cu_seqlens_cmp_kv_val, seqused_q_val, seqused_ori_kv_val, seqused_cmp_kv_val, cmp_residual_kv_val,
        ori_topk_length_val, cmp_topk_length_val, num_heads_q, num_heads_kv, head_dim, batch_size, max_seqlen_q,
...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    ops.impl("npu_sparse_flash_mla_metadata", torch::kPrivateUse1,
             &vllm_ascend::npu_sparse_flash_mla_metadata_npu);
